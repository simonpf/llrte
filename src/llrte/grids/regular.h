...
struct GridPosition {
  ...
  std::array<IndexType, 3> get_index_array() {
    ...
    if (direction.z > 0) {
      ...
      class RegularGrid {
       ...
       bool is_leaving(GridPosition<Vector, Index> &gp) {
         ...
         */
        __DEV__ Index find_next_index(const Vector<Float> &grid, Float p,
                                      Float d) {
          Index i = 1;
         ...
         */
        template <typename Vector>
        __DEV__ GridPosition<Vector, Index> place_on_grid(
        ...
        std::tuple<Index, Float> interpolation_weights_1d(
            const Vector<Float> &grid, Index index, Float position) {
          Float next_grid = 0.0;
        ...
        auto interpolate(const GridPosition<Vector> &gp, const Tensor &t) ->
          ...
          ResultType tll =
              ResultType(t(left_index_x, left_index_y, left_index_z));
          ResultType tlr;
          ResultType trl;
          ResultType trr;
          ...
          if (weight_x != 1.0) {
            tll *= weight_x;
            tll +=
                (1.0 - weight_x) * t(right_index_x, left_index_y, left_index_z);
            ...
            if (weight_z != 1.0) {
              tlr = std::move(
                  ResultType(t(left_index_x, left_index_y, right_index_z)));
              tlr *= weight_x;
              tlr += (1.0 - weight_x) *
                     t(right_index_x, left_index_y, right_index_z);
            }
            if (weight_y != 1.0) {
              trl = std::move(
              ...
              trl *= weight_x;
              trl += (1.0 - weight_x) *
                     t(right_index_x, right_index_y, left_index_z);
            }
            if ((weight_z != 1.0) && (weight_y != 1.0)) {
              ...
              trr *= weight_x;
              trr += (1.0 - weight_x) *
                     t(right_index_x, right_index_y, right_index_z);
            }
          }
         ...
         */
        __DEV__ size_t
        get_boundary_index(GridPosition<Vector, Index> gp) const {
          if (gp.x() <= x_[0]) {
            return 0;
          }
          if (gp.x() >= x_[x_.size()]) {
