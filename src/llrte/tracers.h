...
template <typename Grid>
std::shared_ptr<typename Histogram<Grid>::Float[]> Histogram<Grid>::data_ =
    nullptr;
...
class AbsorptionTracer {
 public:
  ...
  static void initialize(const Grid &grid) {
    ...
    absorption_counts_ = std::shared_ptr<Float[]>(new Float[n]);
    scattering_counts_ = std::shared_ptr<Float[]>(new Float[n]);
    ...
    n = 3 * 2;
    leaving_photons_ = std::shared_ptr<Float[]>(new Float[n]);
    for (size_t i = 0; i < n; ++i) {
      leaving_photons_[i] = 0;
    }

    n = 11;
    scattering_frequencies_ = std::shared_ptr<Float[]>(new Float[n]);
    for (size_t i = 0; i < n; ++i) {
 ...
 private:
  static Float shape_[3];
  static std::shared_ptr<Float[]> absorption_counts_;
  static std::shared_ptr<Float[]> scattering_counts_;
  static std::shared_ptr<Float[]> scattering_frequencies_;
  static std::shared_ptr<Float[]> leaving_photons_;
...
template <typename Grid>
std::shared_ptr<typename Grid::Float[]> AbsorptionTracer<Grid>::absorption_counts_ =
    nullptr;
template <typename Grid>
std::shared_ptr<typename Grid::Float[]> AbsorptionTracer<Grid>::scattering_counts_ =
nullptr;
template <typename Grid>
std::shared_ptr<typename Grid::Float[]> AbsorptionTracer<Grid>::scattering_frequencies_ =
nullptr;
template <typename Grid>
std::shared_ptr<typename Grid::Float[]> AbsorptionTracer<Grid>::leaving_photons_ =
nullptr;
template <typename Grid>
